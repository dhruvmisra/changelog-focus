...
export const LinkForm = ({
    ...
    return (
        <form className="relative mb-4 w-full" onSubmit={handleReleasesSubmit} ref={formRef}>
            <input
                type="text"
                id="repository-link"
                className="peer w-full rounded-lg border border-gray-800 bg-gray-900 p-2.5 text-sm text-gray-50 focus:border-violet-500 focus:ring-violet-500"
            ...
            {isLinkEmpty && (
                <label
                    ...
                    className="pointer-events-none absolute left-0 top-0 ml-3 mt-3 select-none overflow-hidden text-sm text-gray-400 peer-autofill:opacity-0"
            ...
            <button
                type="submit"
                disabled={isLoading}
                className="absolute right-0 top-[1px] w-10 rounded-r-lg bg-violet-600 p-2.5 text-sm font-medium text-white hover:bg-violet-700 focus:outline-none  focus:ring-4 focus:ring-violet-800 disabled:bg-gray-700"
            >
                {isLoading ? (
                    <>
                        <svg
