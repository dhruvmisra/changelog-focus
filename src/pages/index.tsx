...
const Home: NextPage = () => {
    ...
    { repositoryLink },
    {
      ...
      onError: (error) => {
        const repositoryError =
          error.data?.zodError?.fieldErrors.repositoryLink;
        toast.error(
          `Oops, somehting went wrong! ${repositoryError?.join(", ")}`
        );
      },
    }
  );
  ...
  // const sortedReleases = releases?.sort((a, b) => a.created_at < b.created_at ? 1 : -1)
          ...
          <div className="container">
            ...
            <ul className="w-full gap-6 md:grid-cols-3">
              {releases?.map((release, index) => (
                <li
                  className="mb-1"
                  key={`${release.id}-${selectedReleases[index]}`}
                >
                  <input
                    type="checkbox"
                    id={release.name}
            ...
            {releases && firstSelectedReleaseIndex !== -1 && lastSelectedReleaseIndex !== -1 && (
              <button
                type="button"
                className="mt-4 mx-auto block w-full rounded-lg bg-blue-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 sm:w-auto"
              >
